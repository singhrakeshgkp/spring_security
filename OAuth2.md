...
- f everything goes fine, u wil get access token in the response. Make sure each time while generating the token u are passsing unique authorization code otherwise ```invaid grant type``` error will be returned in postman by oauth2 server.
