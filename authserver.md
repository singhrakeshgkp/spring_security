...
- **Explanation Scenario 1** ---> there are two cases
  - **Case 1 After restart no one make request with new token(its first call to resource server with existing token and kid TTL is not passed)** ---> Resource server will give success response, detailed explanation is given below
